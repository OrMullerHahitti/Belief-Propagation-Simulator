...
def get_line_segments(slopes, intercepts):
    ...
    for i in range(len(stack)):
        ...
        segments.append(
            {"start": start, "end": end, "m": stack[i][1], "c": stack[i][2]}
        )
    return segments
...
def plot_curve_intersections(
    cost_matrix, axis=1, edge_values=None, delta_q_range=(-15, 15)
):
        ...
        for j in range(i + 1, n_out):
            ...
            for pt in pts:
                intersections.append(
                    {"x": pt[0], "y": pt[1], "labels": (chr(97 + i), chr(97 + j))}
                )
