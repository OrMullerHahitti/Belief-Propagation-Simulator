"""Plot the split-vs-no-split belief gap on the tail chain with a [5, 0] unary.

Third variant of the split example from `notebooks/split_tail_example.ipynb`:
the belief delta at X1 over iterations for six scenarios (split / no split
crossed with damping 0.0 / 0.5 / 0.9).

Chain: X1 -- F12 -- X2 -- F23 -- X3. Each split half of F12 carries
[[0, 20], [30, 2]], so the full table is [[0, 40], [60, 4]]. F23 is the same
kind of unary-style constraint used before -- a binary table over (X2, X3)
whose row minima make it drive a constant [5, 0] into X2, because X3 is a leaf
and always returns a zero Q-message.

That constant is what makes this case interesting: F12 alone prefers (a, a) at
cost 0 over (b, b) at cost 4, but the extra 5 charged to X2=a flips the global
optimum to (b, b) at cost 4 versus 5. The delta is plotted as b[0] - b[1];
beliefs are costs, so a positive delta means X1 leans to the optimal state b.

PDF is written to --plots-dir; the default 60-iteration run keeps the plain
name and any other horizon gets an it<N> suffix:
  split_tail_unary5.pdf        (60 iterations)
  split_tail_unary5_it100.pdf
  split_tail_unary5_it300.pdf

Example:
  uv run python experiments/aaai/code/plot_split_tail_unary5.py
  uv run python experiments/aaai/code/plot_split_tail_unary5.py --iterations 300
"""
...
# F23 is built so it always sends a constant [5, 0] into X2
C23_UNARY: CostTable = np.array([[5, 5], [0, 5]])
...
def output_name(iterations: int) -> str:
    ...
    if iterations == ITERATIONS:
        return "split_tail_unary5.pdf"
    return f"split_tail_unary5_it{iterations}.pdf"


def plot_tail_unary5(plots_dir: Path, iterations: int) -> Path:
...
def main() -> None:
    ...
    out = plot_tail_unary5(Path(args.plots_dir), args.iterations)
    print(f"wrote {out}")
