...
def _apply_overrides(
    ...
    if args.save_snapshots is not None:
        updates["save_snapshots"] = args.save_snapshots
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="save_snapshots",
        help="Write engine snapshots in addition to traces.",
    )
    parser.add_argument(
        "--no-snapshots",
        action="store_false",
        dest="save_snapshots",
        help="Skip snapshot JSONL output; traces and summaries are still written.",
    )
    parser.set_defaults(normalize_messages=True)
    parser.set_defaults(save_snapshots=None)
