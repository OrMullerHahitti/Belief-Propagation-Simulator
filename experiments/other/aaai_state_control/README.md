...
```sh
uv run --no-sync python -m experiments.other.aaai_state_control.study ablate \
...
uv run --no-sync python -m experiments.other.aaai_state_control.mechanism \
  --out results/state_reproduction/mechanism --seeds 2

uv run --no-sync python -m experiments.other.aaai_state_control.study train \
  --out results/state_reproduction/training \
  --families k4_d10 random_sparse --seed-start 18100 --seeds 12

uv run --no-sync python -m experiments.other.aaai_state_control.study evaluate \
...
uv run --no-sync python -m experiments.other.aaai_state_control.native_replay \
...
uv run --no-sync python -m experiments.other.aaai_state_control.analyze \
