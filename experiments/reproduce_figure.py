...
def _replay_fig8_from_csv(cycle_size: int, csv_dir: Path, out_dir: Path) -> bool:
    ...
    """
    csv_path = csv_dir / f"cycle_{cycle_size}" / "examples.csv"
...
def _run_replay_batch(out_subdir: str, figures: tuple[str, ...] = ("5a", "5b", "8")) -> None:
    """Replay every saved cost-tables JSON for fig5a/5b in experiments/plots/,
    plus fig8 at every cycle size from experiments/figure_8/cycle_{N}/examples.csv.

    pass `figures` to restrict to a subset (e.g. ("8",) for fig8 only).
    """
    base_dir = Path(__file__).resolve().parent / "plots"
    fig8_csv_dir = Path(__file__).resolve().parent / "figure_8"
