...
uv run --no-sync python -m experiments.aamas.adaptive_split_control.code.run \
  --output results/adaptive_split_control_replay
uv run --no-sync python -m experiments.aamas.adaptive_split_control.code.confirm \
  --pilot results/adaptive_split_control_replay \
  --output results/adaptive_split_control_replay/confirmation
uv run --no-sync python -m experiments.aamas.adaptive_split_control.code.symmetry_probe \
  --output results/adaptive_split_control_replay/theory
uv run --no-sync python -m experiments.aamas.adaptive_split_control.code.verify \
  --pilot results/adaptive_split_control_replay
```
