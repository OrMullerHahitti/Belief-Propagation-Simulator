...
def _are_points_collinear(points: np.ndarray) -> bool:
    ...
    for i in range(n_points - 1):
        ...
        if i == 0 or initial_slope is None: # Find first valid slope
            ...
            elif abs(current_slope - initial_slope) > EPSILON * max(1.0, abs(initial_slope)):
                 return False # Slopes differ significantly

    # If loop completes and we found a consistent slope (or all points were identical)
    return True if initial_slope is not None else False # Need at least one non-identical pair
...
def _compute_single_envelope(
    ...
    def build_envelope_from_two_lines(lineA, lineB, op, k_min, k_max):
        ...
        if k_intersect is None:
            ...
            if k_intersect <= k_min + EPSILON:
                envelope_segments.append(EnvelopeSegment(line_dominant_second, k_min, k_max))
            elif k_intersect >= k_max - EPSILON:
                envelope_segments.append(EnvelopeSegment(line_dominant_first, k_min, k_max))
            else:
                envelope_segments.append(EnvelopeSegment(line_dominant_first, k_min, k_intersect))
                envelope_segments.append(EnvelopeSegment(line_dominant_second, k_intersect, k_max))
                intersection_points.append(IntersectionPoint(
                    k=k_intersect, type='weak',
    ...
    if num_effective_lines == 2:
        return build_envelope_from_two_lines(filtered_lines[0], filtered_lines[1], op, k_min, k_max)
    ...
    lines_for_hull = filtered_lines # Keep track of which line corresponds to which point

    try:
        hull = ConvexHull(points, qhull_options='Qt') # Add 'Qt' for triangulation, sometimes helps precision
    except QhullError as e:
        # Check if the error is due to precision/flatness (collinear points)
        if "QH6154" in str(e) or "QH6214" in str(e) or "precision error" in str(e).lower():
             print(f"Warning: ConvexHull failed ({e}). Checking for collinearity among {num_effective_lines} points.")
             if _are_points_collinear(points):
                 ...
                 sorted_collinear_lines = sorted(lines_for_hull, key=lambda line: line.slope)
                 ...
                 print(f"Error: ConvexHull precision error ({e}), but points not detected as collinear.")
                 raise e
        else:
    ...
    if op == 'max':
        ...
        for line in hull_lines_sorted:
             ...
             hull_lines_final.append(Line(line.slope, -line.intercept, line.source_j, line.target_i))
    ...
    if not hull_lines_sorted:
        print("Warning: Hull lines sorted list is empty after successful hull computation.")
    ...
    elif not envelope_segments and hull_lines_sorted: # Only one line formed the hull relevant to the range
    ...
    if not envelope_segments and filtered_lines:
         ...
         print("Warning: Hull processing yielded no segments, using fallback evaluation.")
    ...
    if final_segments:
         if final_segments[0].k_start > k_min + EPSILON:
             final_segments[0] = EnvelopeSegment(final_segments[0].line, k_min, final_segments[0].k_end)
         if final_segments[-1].k_end < k_max - EPSILON:
              final_segments[-1] = EnvelopeSegment(final_segments[-1].line, final_segments[-1].k_start, k_max)
             ...
             if op == 'min':
                 ...
                 if val > best_val + EPSILON:
                     ...
                     chosen_line = line
                      ...
                      if line.slope > chosen_line.slope: # Prefer higher slope for max tie
                           chosen_line = line
          final_segments = [EnvelopeSegment(chosen_line, k_min, k_max)]
...
def compute_piecewise_envelopes(
    ...
    if cost_table.shape[0] != cost_table.shape[1] or cost_table.shape[0] != q.shape[0]:
        raise ValueError("Shapes mismatch: cost_table must be (d, d) and q must be (d,).")
    if k_min >= k_max:
    ...
    for i in range(d): # Target state i
        ...
        for j in range(d): # Source state j
            ...
            intercept = q[j]
            lines_for_i.append(Line(slope=slope, intercept=intercept, source_j=j, target_i=i))
        ...
        if not segments:
             print(f"Warning: No segments returned for individual envelope E_{i}. This might indicate an issue.")
    ...
    sorted_breakpoints = sorted(list(b for b in all_breakpoints if math.isfinite(b))) # Ensure finite
    ...
    for idx in range(len(filtered_breakpoints) - 1):
        ...
        if k_end <= k_start + EPSILON: continue

        k_mid = (k_start + k_end) / 2.0
        # Ensure k_mid is within bounds strictly
        if k_mid < k_min: k_mid = k_min + EPSILON * (k_max - k_min)
        if k_mid > k_max: k_mid = k_max - EPSILON * (k_max - k_min)
        # If interval is tiny, k_mid might equal k_start or k_end, adjust slightly
        if abs(k_mid - k_start) < EPSILON: k_mid = k_start + EPSILON * (k_end-k_start)
        if abs(k_mid - k_end) < EPSILON: k_mid = k_end - EPSILON * (k_end-k_start)
        ...
        for i in range(d):
             if not individual_envelopes[i]: continue
             ...
             if active_segment_for_i:
                 ...
                 if math.isnan(current_val): continue
                 ...
                 if is_better:
                     ...
                     current_winning_target_i_for_interval = active_segment_for_i.line.target_i
                      ...
                      if winning_segment is None:
                           ...
                           if current_line.source_j < existing_line.source_j:
                               winning_segment = active_segment_for_i
                               current_winning_target_i_for_interval = active_segment_for_i.line.target_i
                           elif current_line.source_j == existing_line.source_j and current_line.target_i < existing_line.target_i:
                               winning_segment = active_segment_for_i
                               current_winning_target_i_for_interval = active_segment_for_i.line.target_i


        if winning_segment:
            current_winning_line_info = (winning_segment.line.source_j, winning_segment.line.target_i)

            if last_winning_target_i is not None and \
               current_winning_target_i_for_interval is not None and \
               current_winning_target_i_for_interval != last_winning_target_i and \
               k_start > k_min + EPSILON:
                 point_found = False
                 for p in all_intersection_points:
                     if abs(p.k - k_start) < EPSILON:
                          ...
                          break
                 if not point_found:
                     all_intersection_points.append(IntersectionPoint(k=k_start, type='strong'))


            # Merge or add segment
            if final_envelope and \
               final_envelope[-1].line == winning_segment.line and \
               abs(final_envelope[-1].k_end - k_start) < EPSILON :
                 final_envelope[-1] = EnvelopeSegment(
                     final_envelope[-1].line,
                     final_envelope[-1].k_start,
                     k_end
                 )
            else:
                 final_envelope.append(EnvelopeSegment(
                    winning_segment.line,
                    k_start, # Start at the breakpoint
                    k_end
                 ))
            ...
            if final_envelope: # Attempt to extend the previous segment if possible
                print(f"Warning: No winning segment found for interval [{k_start:.4f}, {k_end:.4f}]. Extending previous segment if possible.")
                # Check if previous segment's line is valid at k_mid
                prev_seg = final_envelope[-1]
                prev_val = prev_seg.line.evaluate(k_mid)
                if math.isfinite(prev_val): # If previous line is valid, tentatively extend it
                     final_envelope[-1] = EnvelopeSegment(prev_seg.line, prev_seg.k_start, k_end)
                     # Keep last_winning info the same
                else: # Cannot extend, discontinuity or error
                     print(f"Error: Cannot extend previous segment in gap interval [{k_start:.4f}, {k_end:.4f}].")
                     last_winning_line_info = None
                     last_winning_target_i = None

            else: # No previous segment to extend, serious issue
                print(f"Error: No winning segment found for initial interval [{k_start:.4f}, {k_end:.4f}]. Cannot build final envelope.")
                # Possibly return empty or raise error depending on desired behavior
                return individual_envelopes, [], all_intersection_points # Return partial results
    ...
    if final_envelope:
        if final_envelope[0].k_start > k_min + EPSILON:
            final_envelope[0] = EnvelopeSegment(final_envelope[0].line, k_min, final_envelope[0].k_end)
        if final_envelope[-1].k_end < k_max - EPSILON:
            final_envelope[-1] = EnvelopeSegment(final_envelope[-1].line, final_envelope[-1].k_start, k_max)
...
if __name__ == "__main__":
    ...
    cost_table_ex1 = np.array([
        [1., 5.],  # j=0 -> C[0,0]=1, C[0,1]=5
        [3., 2.]   # j=1 -> C[1,0]=3, C[1,1]=2
    ])
    q_ex1 = np.array([4., 1.]) # q[0]=4, q[1]=1
    ...
    for i, env in enumerate(ind_env1):
        print(f" E_{i}:")
        for seg in env: print(f"  {seg}")
    print("\nFinal Envelope:")
    for seg in final_env1: print(f"  {seg}")
    print("\nIntersection Points:")
    if not points1: print("  (None)")
    for p in points1: print(f"  {p}")
    ...
    for i, env in enumerate(ind_env2):
        ...
        for seg in env: print(f"  {seg}")
    print("\nFinal Envelope:")
    for seg in final_env2: print(f"  {seg}")
    print("\nIntersection Points:")
    if not points2: print("  (None)")
    for p in points2: print(f"  {p}")

    print("\n\n--- Example 3 (d=2, min, k in [0, 5] - potential intersections) ---")
    cost_table_ex3 = np.array([
        [1., 5.],
        [3., 2.]
    ])
    q_ex3 = np.array([4., 1.])
    ...
    for i, env in enumerate(ind_env3):
        ...
        for seg in env: print(f"  {seg}")
    print("\nFinal Envelope:")
    for seg in final_env3: print(f"  {seg}")
    print("\nIntersection Points:")
    if not points3: print("  (None)")
    for p in points3: print(f"  {p}")
    ...
    for i, env in enumerate(ind_env4):
        ...
        for seg in env: print(f"  {seg}")
    print("\nFinal Envelope:")
    for seg in final_env4: print(f"  {seg}")
    print("\nIntersection Points:")
    if not points4: print("  (None)")
    for p in points4: print(f"  {p}")
