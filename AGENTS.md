...
Target Python 3.10+, keep modules ASCII, and let Black format code (complements `flake8`’s 120-character enforcement). Use `snake_case` for functions, modules, and variables; reserve `PascalCase` for classes and `UPPER_CASE` for constants. Annotate public APIs with type hints and docstrings, and isolate reusable helpers inside `src/propflow` to avoid import cycles.
...
Commits follow Conventional Commit prefixes (`feat`, `fix`, `refactor`, `test`, `chore`) and focus on one logical change. Reference issues or key configs in the subject when helpful. PRs need a clear problem statement, reproduction steps or config/seed, and refreshed logs or screenshots for snapshot analysis outputs. Confirm `pre-commit run -a` and the full test suite pass, and keep artifacts and `__pycache__` out of Git.
