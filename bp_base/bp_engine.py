...
class BPEngine:
    ...
    def run(self, max_iter: int = 1000, save_json: bool = True, filename: str = None) -> None:
        """
        ...
        if save_json:
            self.history.save_results("results.json")  # TODO ; needs to be changed to a more general name
        return None
### -------------------------------------------------------------------####
    ...
    def calculate_global_cost(self) -> float:
        ...
        var_assignments = {node: node.curr_assignment for node in self.graph.G.nodes() if isinstance(node, VariableAgent)}
