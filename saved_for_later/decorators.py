def validate_message_direction(func):
    ...
    - If message is from variable 1 to 2 with name F12, processes normally
    - Otherwise transposes the message first
    """
    ...
    def wrapper(self, message, *args, **kwargs):
        ...
        if sender_name.startswith('V') and recipient_name.startswith('F'):
            ...
            if not factor_name.startswith('F') or var_id not in factor_name[1:]:
                # Transpose message if direction doesn't match naming convention
                message.message = message.message.T
        ...
        # Call the original function with possibly transformed message
