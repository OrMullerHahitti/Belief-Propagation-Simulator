import numpy as np
import pandas as pd
from global_mapping import number_to_letter
# Sample values
def create_random_df(col_and_row_num=3):
    df = pd.DataFrame(np.random.randint(0, 10, size=(col_and_row_num, col_and_row_num)))
    df = df.T
    return df

def add_index_labels(df):
    index_labels = list(range(1, len(df) + 1))
    df.index = index_labels
    df.columns = index_labels
    return df
# TODO implemet all the get and sets for the Q class so itll be easy to use
import numpy as np
import pandas as pd
from global_mapping import number_to_letter

class Q(np.ndarray):
    def __new__(cls, df, values=None):
        obj = np.ones(len(df.columns) + 1).view(cls)  # Start with 1 and include a dummy value at index 0
        obj.df = df
        if values is not None:
            obj[1:] = values
        obj.labels = {number_to_letter[i]: i for i in range(1, len(df.columns) + 1)}
        return obj
    ...
    def __repr__(self):
        return f"Q(values={super().__repr__()}, labels={self.labels})"
...
def C(col, row, df=df):
    return df[col][row]
# TODO - Implement it in a way that it can be either aggregated by rows or columns depends
def calc_R(Q=Q,df=df):
    R = [[] if i != 0 else None for i in range(len(Q))]

    for j in range(1, len(Q)):
        R[j] = [None] + []
        for i in range(1, len(Q)):
            R[j].append(Q[i]+C(j,i))
    return R
# Example usage
print(C(1, 2))
R_all=calc_R()
class recieved:
    def __init__(self, R=R_all):
        self.R = R
