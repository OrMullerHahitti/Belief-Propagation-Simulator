...
def update_version_py(version_file: Path, new_version: str) -> None:
    ...
    pattern = re.compile(r'(?m)^(?P<prefix>__version__\s*=\s*")(?P<version>[^"]+)(?P<suffix>")')
    match = pattern.search(text)
    if not match:
...
def update_docs_conf(conf_file: Path, new_version: str) -> None:
    ...
    # Update version
    version_pattern = re.compile(r"(?m)^(?P<prefix>version\s*=\s*')(?P<version>[^']+)(?P<suffix>')")
    version_match = version_pattern.search(text)
    if version_match:
        text = version_pattern.sub(f"\\g<prefix>{new_version}\\g<suffix>", text, count=1)
    else:
        print(f"Warning: version not found in {conf_file}")
    
    # Update release
    release_pattern = re.compile(r"(?m)^(?P<prefix>release\s*=\s*')(?P<version>[^']+)(?P<suffix>')")
    release_match = release_pattern.search(text)
    if release_match:
        text = release_pattern.sub(f"\\g<prefix>{new_version}\\g<suffix>", text, count=1)
    else:
