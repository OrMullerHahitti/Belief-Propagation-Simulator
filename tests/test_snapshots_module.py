...
def sample_engine(sample_factor_graph):
    class DummyEngine:
        def __init__(self, graph):
            ...
            self._assignments = {var.name: 0 for var in graph.variables}
        ...
        def get_beliefs(self):
            return {var.name: np.ones(var.domain) for var in self.var_nodes}
...
def test_build_snapshot_from_engine(sample_engine, sample_step):
    ...
    assert snapshot.global_cost == pytest.approx(0.0)


def test_snapshot_manager_capture(sample_engine, sample_step):
    manager = SnapshotManager()
    snapshot = manager.capture_step(0, sample_step, sample_engine)
    assert isinstance(snapshot, EngineSnapshot)
