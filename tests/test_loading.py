...
from utils.loading_utils import load_factor_graph # Assuming this loading function exists
...
TEST_WORKSPACE_ROOT = "/workspaces/Belief-Propagation-Simulator/" 
...
def factor_graph_pickle_path():
    ...
    path = os.path.join(TEST_WORKSPACE_ROOT, "configs", "factor_graphs", "test_factor_graph.pkl")
...
def test_load_factor_graph_from_pickle(factor_graph_pickle_path):
    ...
    try:
        loaded_fg = load_factor_graph(factor_graph_pickle_path)
    ...
    assert loaded_fg.name is not None, "Loaded factor graph should have a name"
