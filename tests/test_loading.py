...
def factor_graph_pickle_path():
    ...
    path = os.path.join(
        TEST_WORKSPACE_ROOT, "configs", "factor_graphs", "test-factor.pkl"
    )
...
def test_load_factor_graph_from_pickle(factor_graph_pickle_path):
    ...
    try:
        ...
        pytest.fail(
            f"Test factor graph pickle file not found at {factor_graph_pickle_path}. "
    ...
    assert isinstance(
        loaded_fg, FactorGraph
    ), "Loaded object should be an instance of FactorGraph"
