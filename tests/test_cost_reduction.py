...
class TestCostReductionPolicy:
    ...
    def test_every_time_cost_reduction(self):
        ...
        for iteration in iterations_to_test:
            ...
            logger.info(f"Iteration {iteration} - EXPECTED: {expected}, ACTUAL: {actual}")
    ...
    def test_min_max_envelope_cost_reduction(self):
        ...
        for iteration in even_iterations:
            expected = True
            actual = policy.should_apply(iteration)
            logger.info(f"Even iteration {iteration} - EXPECTED: {expected}, ACTUAL: {actual}")
            assert actual == expected, f"should_apply failed for even iteration {iteration}"
        
        for iteration in odd_iterations:
            expected = False
            actual = policy.should_apply(iteration)
            logger.info(f"Odd iteration {iteration} - EXPECTED: {expected}, ACTUAL: {actual}")
            assert actual == expected, f"should_apply failed for odd iteration {iteration}"
    ...
    def test_envelope_based_cost_reduction(self):
        ...
        custom_policy = EnvelopeBasedCostReduction(
            applying_critiria=lambda x: x % 2 == 1,
            min_k=0.1,
            max_k=0.9
        )
        ...
        for iteration in even_iterations:
            ...
            logger.info(f"Custom policy, even iteration {iteration} - EXPECTED: {expected}, ACTUAL: {actual}")
            assert actual == expected, f"Custom policy should_apply failed for even iteration {iteration}"
        
        for iteration in odd_iterations:
            expected = True
            actual = custom_policy.should_apply(iteration)
            logger.info(f"Custom policy, odd iteration {iteration} - EXPECTED: {expected}, ACTUAL: {actual}")
            assert actual == expected, f"Custom policy should_apply failed for odd iteration {iteration}"
        ...
        assert k_value >= policy.min_k and k_value <= policy.max_k, \
    ...
    def test_get_optimal_k_segments(self):
        ...
        for i, segment in enumerate(segments):
            ...
            logger.info(f"Segment {i}: k range [{k_start:.4f}, {k_end:.4f}], "
