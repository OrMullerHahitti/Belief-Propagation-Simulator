...
def test_create_ndarray_values():
    ...
    logger.info(
        f"EXPECTED result dtype is integer: {expected_is_integer}, ACTUAL: {actual_is_integer}"
    )
    ...
    logger.info(
        f"EXPECTED min value >= {expected_min_range}, ACTUAL min value: {actual_min}"
    )
    assert (
        actual_min >= expected_min_range
    ), f"Expected min value >= {expected_min_range}, got {actual_min}"

    # Check for max value ≤ 9
    expected_max_range = 9
    actual_max = np.max(result)
    logger.info(
        f"EXPECTED max value <= {expected_max_range}, ACTUAL max value: {actual_max}"
    )
    assert (
        actual_max <= expected_max_range
    ), f"Expected max value <= {expected_max_range}, got {actual_max}"
...
def test_create_ndarray_different_calls():
    ...
    logger.info(
        f"Testing randomness for domain={domain}, num_connected={num_connected}"
    )
    ...
    assert (
        not arrays_equal
    ), "Expected different arrays on subsequent calls, but got identical arrays"
