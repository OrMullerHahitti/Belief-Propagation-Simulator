...
def test_constant_damping_policy_logs():
    ...
    last_messages = [
        Message(data=np.array([1.0, 2.0, 3.0]), sender=var, recipient=var)
    ]
    inbox_messages = [
    ...
    logger.info(f"Before damping: last_iteration={ [m.data for m in var.last_iteration] }, inbox={ [m.data for m in var.mailer.inbox] }")
    ...
    assert np.array_equal(damped[0].data, expected[0]), f"Expected {expected}, got {damped}"