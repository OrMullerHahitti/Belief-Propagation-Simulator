...
log_file = os.path.join(
    log_dir, f"factor_graph_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
...
def test_factor_graph_initialization():
    ...
    # Create edges
    edges = {factor12: [var1, var2], factor23: [var2, var3]}

    # Initialize FactorGraph
    fg = FactorGraph(
        variable_li=[var1, var2, var3], factor_li=[factor12, factor23], edges=edges
    )
    ...
    assert all(
        edge in actual_edges or tuple(reversed(edge)) in actual_edges
...
def test_initialize_cost_table():
    ...
    assert factor12.cost_table.shape == (
        2,
        2,
    )  # Based on 2 variables with domain size 2
