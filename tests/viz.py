...
def generate_elements(G):
    ...
    elements += [{"data": {"source": str(edge[0]), "target": str(edge[1])}} for edge in G.edges()]
    return elements
...
app.layout = html.Div([
    ...
    cyto.Cytoscape(
        ...
        stylesheet=[
            {"selector": "node", "style": {"content": "data(label)", "background-color": "#3498db", "width": 25, "height": 25}},
...
def update_graph(n_nodes, n_edges, elements):
    ...
    for element in elements:
        if "source" in element["data"]:  # It's an edge
            G_dynamic.add_edge(int(element["data"]["source"]), int(element["data"]["target"]))
        else:  # It's a node
