...
def create_simple_factor_graph():
    ...
    factor = FactorAgent(name="factor", domain=2, ct_creation_func=lambda **kw: cost_table.copy())
...
def test_equivalence_split_vs_costreduction():
    ...
    assert split_msgs_flat == costred_msgs_flat, f"Factor messages differ: {split_msgs_flat} vs {costred_msgs_flat}"
    # Compare variable beliefs
    for i, (v1, v2) in enumerate(zip(fg1.variables, fg2.variables)):
        np.testing.assert_allclose(v1.belief, v2.belief, err_msg=f"Belief mismatch at variable {i}")

if __name__ == "__main__":
    test_equivalence_split_vs_costreduction()
    print("Test passed: The two engines produce identical messages and beliefs after one iteration.")