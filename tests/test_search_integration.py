...
from propflow.search import DSAComputator, MGMComputator
from propflow.search import SearchVariableAgent
...
def test_search_agent_integration():
    ...
    for step in range(3):
        ...
        print(
            f"Step {step+1}: x1: {agent1.curr_assignment} -> {decision_1}, x2: {agent2.curr_assignment} -> {decision_2}"
        )
    ...
    for step in range(3):
        ...
        print(
            f"  Decisions: x1: {agent1.curr_assignment} -> {decision_1}, x2: {agent2.curr_assignment} -> {decision_2}"
        )
...
def test_algorithm_convergence():
    ...
    factor1 = MockFactor(
        "f12", [[2.0, 1.0], [3.0, 4.0]], {"x1": 0, "x2": 1}
    )  # Prefers (0,1)
    factor2 = MockFactor(
        "f23", [[4.0, 1.0], [2.0, 3.0]], {"x2": 0, "x3": 1}
    )  # Prefers (1,0)
