...
class TestFGBuilder:
    ...
    def test_build_cycle_graph_basic(self, domain_size, num_vars, cost_table_config):
        ...
        assert len(fg.factors) == num_vars  # Cycle has same number of factors as variables
    ...
    def test_build_random_graph_basic(self, domain_size, num_vars, density, cost_table_config):
    ...
    def test_cycle_graph_structure(self):
        ...
        for var in fg.variables:
            connected_factors = [f for f, vars_list in fg.edges.items() if var in vars_list]
    ...
    def test_random_graph_density_effect(self):
        ...
        assert len(fg_high.factors) >= len(fg_low.factors) or len(fg_high.factors) >= num_vars // 2
    ...
    def test_graph_networkx_structure(self):
        ...
        assert len(fg.G.edges()) == sum(len(vars_list) for vars_list in fg.edges.values())