...
def run_single_simulation(args):
    ...
    print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid} (thread {tid}): Starting simulation")
    sys.stdout.flush()
    ...
    try:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Args unpacked - graph {graph_index}, engine {engine_name}")
        sys.stdout.flush()

        # Check memory before unpickling
        process = psutil.Process()
        memory_before = process.memory_info().rss / 1024 / 1024  # MB
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Memory before unpickling: {memory_before:.1f} MB")
        sys.stdout.flush()

        # Recreate graph from pickled data
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Starting pickle.loads()...")
        sys.stdout.flush()
        ...
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Child processes after unpickling: {len(children)}")
        sys.stdout.flush()

        # Instantiate the engine
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Creating engine {engine_name}...")
        sys.stdout.flush()
        start_time = time_module.time()
        ...
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Starting engine.run() with max_iter={max_iter}...")
        sys.stdout.flush()
        start_time = time_module.time()
        ...
        def monitored_run(*args, **kwargs):
            print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: engine.run() called with args={args}, kwargs={kwargs}")
            sys.stdout.flush()
            ...
            if hasattr(engine, 'step'):
                ...
                def monitored_step(*step_args, **step_kwargs):
                    ...
                    if current_iter % 50 == 0 or current_iter <= 10:  # Log every 50 iterations + first 10
                        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Iteration {current_iter}/{max_iter}")
                        sys.stdout.flush()
                    ...
                    if current_iter > max_iter + 100:  # Safety check
                        ...
                        sys.stdout.flush()
                        raise RuntimeError(f"Iteration count {current_iter} exceeded max_iter {max_iter}")
        ...
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Exception type: {type(e).__name__}")
        print(f"[{time.strftime('%H:%M:%S')}] PROCESS {pid}: Exception message: {str(e)}")
...
def run_batch_safe(simulation_args, max_workers=None):
    ...
    print(f"[{time.strftime('%H:%M:%S')}] MAIN: Main process memory: {main_memory:.1f} MB")

    # Strategy 1: Try full multiprocessing first
    try:
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: Attempting full multiprocessing with {max_workers} processes...")
        ...
        with Pool(processes=max_workers) as pool:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] MAIN: map_async started, waiting for results with 600s timeout...")
        ...
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: SUCCESS - Full multiprocessing completed in {total_time:.2f}s")
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: Got {len(all_results)} results")
        return all_results

    except Exception as e:
        elapsed = time.time() - start_time if 'start_time' in locals() else 0
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: FULL MULTIPROCESSING FAILED after {elapsed:.2f}s")
...
def run_in_batches(simulation_args, batch_size=None, max_workers=None):
    ...
    print(f"[{time.strftime('%H:%M:%S')}] BATCH: Total simulations: {len(simulation_args)}")
    ...
    for i in range(0, len(simulation_args), batch_size):
        ...
        print(f"[{time.strftime('%H:%M:%S')}] BATCH {batch_num}/{num_batches}: Starting ({len(batch)} simulations)")

        try:
            workers_for_batch = min(max_workers, len(batch))
            print(f"[{time.strftime('%H:%M:%S')}] BATCH {batch_num}: Using {workers_for_batch} workers")
            ...
            with Pool(processes=workers_for_batch) as pool:
                ...
                print(f"[{time.strftime('%H:%M:%S')}] BATCH {batch_num}: Completed in {batch_time:.2f}s")
            ...
            print(f"[{time.strftime('%H:%M:%S')}] BATCH {batch_num}: FAILED - {type(e).__name__}: {str(e)}")

            # Fallback to sequential for this batch
            print(f"[{time.strftime('%H:%M:%S')}] BATCH {batch_num}: Running sequentially as fallback...")

            for j, args in enumerate(batch):
                try:
                    print(f"[{time.strftime('%H:%M:%S')}] BATCH {batch_num}: Sequential {j + 1}/{len(batch)}")
                    result = run_single_simulation(args)
                    all_results.append(result)
                except Exception as seq_e:
                    print(f"[{time.strftime('%H:%M:%S')}] BATCH {batch_num}: Sequential item {j + 1} FAILED: {seq_e}")

    print(f"[{time.strftime('%H:%M:%S')}] BATCH: All batches completed, total results: {len(all_results)}")
    return all_results
...
if __name__ == '__main__':
    ...
    for i, graph in enumerate(random_fg):
        # Pickle the graph once for all engines
    ...
    print(f"[{time.strftime('%H:%M:%S')}] MAIN: Multiprocessing start method: {mp.get_start_method()}")

    # Add system info
    import psutil

    print(f"[{time.strftime('%H:%M:%S')}] MAIN: System memory: {psutil.virtual_memory().total / 1024 ** 3:.1f} GB")
    print(
    ...
    try:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: CRITICAL ERROR - All multiprocessing strategies failed!")
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: Exception type: {type(e).__name__}")
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: Exception message: {str(e)}")
        import traceback
        ...
        print(f"[{time.strftime('%H:%M:%S')}] MAIN: Falling back to sequential processing...")
        all_results = sequential_fallback(simulation_args)

    end_time = time.time()
    total_time = end_time - start_time

    print(f"[{time.strftime('%H:%M:%S')}] All simulations completed in {total_time:.2f} seconds")
    print(f"[{time.strftime('%H:%M:%S')}] Average time per simulation: {total_time / total_simulations:.2f} seconds")

    # Verify we got all results
    if len(all_results) != total_simulations:
        print(f"[{time.strftime('%H:%M:%S')}] WARNING: Expected {total_simulations} results, got {len(all_results)}")
    ...
    for engine_name, costs_list in results.items():
        print(f"[{time.strftime('%H:%M:%S')}] {engine_name}: {len(costs_list)} runs completed")
    ...
    for idx, (engine_name, costs_list) in enumerate(results.items()):
        if not costs_list:
            print(f"[{time.strftime('%H:%M:%S')}] WARNING: No results for {engine_name}")
            continue
        ...
        print(f"[{time.strftime('%H:%M:%S')}] Plotted {engine_name}: avg final cost = {avg_costs[-1]:.2f}")

    plt.title(f"Average Costs over {len(random_fg)} Runs on Random Factor Graphs (Density 0.25)\n"
              f"Total runtime: {total_time:.1f}s using {cpu_count()} CPU cores", fontsize=14)
    ...
    plt.figtext(0.02, 0.02, f"Simulations: {total_simulations} | "
