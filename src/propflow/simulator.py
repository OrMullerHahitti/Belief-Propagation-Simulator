...
def _setup_logger(level=None):
    ...
    if level is None:
        level = SIMULATOR_DEFAULTS["default_log_level"]
...
class Simulator:
    def __init__(self, engine_configs, log_level=None):
    ...
    def run_simulations(self, graphs, max_iter=None):
        # Use centralized default if not provided
        if max_iter is None:
            max_iter = SIMULATOR_DEFAULTS["default_max_iter"]
    ...
    def plot_results(self, max_iter=None, verbose=False):
        # Use centralized default if not provided
        if max_iter is None:
            max_iter = SIMULATOR_DEFAULTS["default_max_iter"]
    ...
    def _run_single_simulation(args):
        ...
        if not logger.handlers:
            ...
            formatter = logging.Formatter(
                f"[%(asctime)s] [%(levelname)s] [p%(process)d] %(message)s", "%H:%M:%S"
            )
        ...
        try:
            logger.debug(
                f"Starting simulation for graph {graph_index}, engine {engine_name}"
            )
            ...
            logger.info(
                f"Finished simulation for graph {graph_index}, engine {engine_name}. Final cost: {costs[-1] if costs else 'N/A'}"
            )

            return (graph_index, engine_name, costs)
        except Exception as e:
            logger.error(
                f"Exception in simulation for graph {graph_index}, engine {engine_name}: {e}"
            )
    ...
    def _run_in_batches(self, simulation_args, batch_size=None, max_workers=None):
        ...
        for i in range(0, len(simulation_args), batch_size):
            ...
            try:
                ...
                self.logger.error(
                    f"Batch {batch_num} failed: {e}. Running sequentially as fallback."
                )
                for args in batch:
                    try:
                        ...
                        self.logger.error(
                            f"Sequential item failed in batch {batch_num}: {seq_e}"
                        )
