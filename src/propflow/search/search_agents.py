...
from src.propflow.base_models.agents import VariableAgent as BaseVariableAgent
...
class SearchVariableAgent(BaseVariableAgent):
    ...
    def compute_search_step(self, neighbors_values: Optional[Dict[str, Any]] = None) -> Optional[Any]:
        """
    ...
    def update_assignment(self):
        ...
        if self._pending_assignment is not None:
            ...
            if self._pending_assignment != self.curr_assignment:
                ...
                logger.debug(f"Agent {self.name} changed from {old_value} to {self._pending_assignment}")
