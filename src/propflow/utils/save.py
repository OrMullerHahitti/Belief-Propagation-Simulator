...
def save_simulation_data(engine: Any, filepath: str) -> str:
    ...
    for idx, agent in enumerate(engine.graph.variables):
        domain_values = (
            [str(i) for i in range(agent.domain)]
    ...
    for idx, factor in enumerate(engine.graph.factors):
        ...
        connected_agents = [
            ...
            for name in getattr(factor, "connection_number", {}).keys()
        ]
        factor_type = "binary" if len(connected_agents) > 1 else "unary"
        default_cost = (
            float(factor.cost_table.mean())
    ...
    if hasattr(engine, "history"):
        ...
        if hasattr(history, "beliefs"):
            history_data["beliefs_summary"] = {
                str(k): {an: _serialize_numpy_array(b) for an, b in v.items()}
                for k, v in history.beliefs.items()
            }
        if hasattr(history, "assignments"):
            history_data["assignments"] = {
                str(k): {an: int(a) for an, a in v.items()}
                for k, v in history.assignments.items()
            }
...
def save_simulation_result(engine: Any, filepath: str) -> str:
    ...
    if convergence_monitor and hasattr(convergence_monitor, "convergence_history"):
        convergence_achieved = any(
            h.get("belief_converged") and h.get("assignment_converged")
    ...
    simulation_result = {
        ...
        "convergenceAchieved": convergence_achieved,
        "executionTime": 0,
        "messageCount": 0,
    }
    metrics = {
...
class EnhancedSaveModule:
    ...
    def save_simulator_analysis(
        self, simulator: Any, filepath: Optional[str] = None, save_csv: bool = True
    ) -> str:
    ...
    def _extract_simulator_data(self, simulator: Any) -> SimulatorAnalysisData:
        ...
        for engine_name, costs_list in simulator.results.items():
            ...
            engine_stats[engine_name] = {
                ...
                "average_iterations": float(
                    np.mean([len(c) for c in valid_costs_list])
                ),
            ...
            for costs in valid_costs_list:
                if len(costs) > 1:
                    ...
                    if costs[0] > 0:
                        improvement_rates.append((costs[0] - costs[-1]) / costs[0])
            convergence_analysis[engine_name] = {
                ...
                "average_convergence_time": float(
                    np.mean(convergence_times) if convergence_times else 0.0
                ),
                "std_convergence_time": float(
                    np.std(convergence_times) if convergence_times else 0.0
                ),
                "average_improvement_rate": float(
                    np.mean(improvement_rates) if improvement_rates else 0.0
                ),
                "cost_reduction_consistency": float(
                    np.std(improvement_rates) if improvement_rates else 0.0
                ),
            }
            cost_convergence_comparison[engine_name] = np.mean(
                all_costs_array, axis=0
            ).tolist()
        ...
        return SimulatorAnalysisData(
            ...
            total_runs=sum(len(cl) for cl in simulator.results.values()),
            graph_count=len(next(iter(simulator.results.values()), [])),
            timestamp=self.timestamp,
            config_summary={
                "bp": list(simulator.engine_configs.keys()),
                "log_level": getattr(
                    getattr(simulator, "logger", {}), "level", "unknown"
                ),
            },
        )

    def _extract_enhanced_engine_data(
        self, engine: Any, include_performance: bool, include_convergence_detail: bool
    ) -> Dict[str, Any]:
        ...
        if (
            ...
            try:
                enhanced_data["performance_analysis"] = {
                    "summary": engine.performance_monitor.get_summary(),
                    "has_detailed_metrics": True,
                }
            except Exception as e:
                enhanced_data["performance_analysis"] = {
                    "error": f"Could not extract performance data: {e}",
        ...
        if (
            ...
            and engine.history
            and hasattr(engine.history, "costs")
        ):
    ...
    def _get_base_engine_data(self, engine: Any) -> Dict[str, Any]:
        ...
        return save_simulation_data(
            engine, "temp.json"
        )  # Re-uses existing logic by temporary save
    ...
    def _pad_and_convert_costs(self, costs_list: List[List[float]]) -> np.ndarray:
        ...
        return np.array(
            [costs + [costs[-1]] * (max_len - len(costs)) for costs in costs_list]
        )
    ...
    def _calculate_performance_rankings(
        ...
        return {
            engine: {
                "final_cost_rank": next(
                    j for j, (name, _) in enumerate(final_costs) if name == engine
                )
                + 1,
                "convergence_rate_rank": next(
                    j for j, (name, _) in enumerate(conv_rates) if name == engine
                )
                + 1,
                "overall_score": (
                    next(j for j, (name, _) in enumerate(final_costs) if name == engine)
                    + 1
                    + next(
                        j for j, (name, _) in enumerate(conv_rates) if name == engine
                    )
        ...
        if hasattr(engine.convergence_monitor, "convergence_history"):
            ...
            for i, entry in enumerate(engine.convergence_monitor.convergence_history):
                if isinstance(entry, dict):
                    analysis["convergence_events"].append(
                        {
                            "iteration": i,
    ...
    def _analyze_message_patterns(self, history: Any) -> Dict[str, Any]:
        ...
        unique_flows = {
            ...
            for msgs in history.step_messages.values()
            for msg in msgs
        }
        return {
    ...
    def _save_simulator_csv(
        self, analysis_data: SimulatorAnalysisData, filepath: str
    ) -> None:
...
def save_simulator_comprehensive_analysis(
    simulator: Any, filepath: Optional[str] = None, save_csv: bool = True
) -> str:
...
def save_enhanced_engine_analysis(
    ...
    """
    saver = EnhancedSaveModule()
    return saver.save_enhanced_engine_analysis(
