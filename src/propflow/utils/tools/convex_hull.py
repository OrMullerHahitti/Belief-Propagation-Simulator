...
def compute_convex_hull_from_lines(
    ...
    points = np.array(
        [
            [line.slope, -line.intercept if hull_type == "upper" else line.intercept]
            for line in lines
        ]
    )
...
def convex_hull_from_agents(
    ...
    lines = create_lines_from_cost_table(
        factor_agent.cost_table, q_values, k_min, k_max
    )
            ...
            if k_min <= k_val <= k_max:
                intercepts.append(
                    InterceptPoint(
                        ...
                        envelope2_id=hull_result.envelope_id,
                        line1=line1,
                        line2=line2,
                    )
                )
...
def compute_hierarchical_envelopes(
    ...
    partial_change_points = [
        ...
        for icept in find_all_envelope_intercepts(hull)
    ]
    ...
    for hull in individual_hulls:
        for line in hull.hull_lines:
            meta_line = ConvexLine(
                ...
                cell_i=hull.envelope_id,
                cell_j=len(meta_lines),
            )
        ...
        if line1.cell_i != line2.cell_i:
            ...
            if intersection:
                ...
                if k_min <= k_val <= k_max:
                    ...
                    change_assignment_points.append(
                        InterceptPoint(
                            k=k_val,
                            intersection_value=intersection_value,
                            type="change_assignment",
                            envelope1_id=line1.cell_i,
                            envelope2_id=line2.cell_i,
                            line1=orig_line1,
                            line2=orig_line2,
                        )
                    )

    all_intercepts = sorted(
        partial_change_points + change_assignment_points, key=lambda x: x.k
    )
...
def evaluate_hull_at_k(
    ...
    if not valid_values:
        return float("inf"), hull_result.hull_lines[0]
...
def plot_convex_hull(
    ...
    if show_all_lines:
        for i, line in enumerate(hull_result.lines):
            ax.plot(
                k_vals,
                [line.evaluate(k) for k in k_vals],
                "--",
                alpha=0.3,
                color="gray",
...
def plot_intercept_summary(
    ...
    if change_points:
        ax.scatter(
            [p.k for p in change_points],
            [p.intersection_value for p in change_points],
            c="red",
            s=100,
            marker="o",
    ...
    if partial_points:
        ax.scatter(
            [p.k for p in partial_points],
            [p.intersection_value for p in partial_points],
            c="blue",
            s=80,
            marker="^",
...
if __name__ == "__main__":
    ...
    def example_usage():
        ...
        print(
            f"Meta-envelope has {len(hierarchical_result.meta_envelope.hull_lines)} lines on hull"
        )
        if HAS_MATPLOTLIB:
